...
} from "src/utils/intersect";
import {
	DRAWCANVAS_CLASSNAME,
	SELECTIONCANVAS_CLASSNAME,
...
const useShapes = (
	...
	const [selectedShape, setSelectedShape] = useState<ShapeEntity | undefined>(
		undefined,
	);
	const [hoveredShape, setHoveredShape] = useState<ShapeEntity | undefined>(
		undefined,
	);
	...
	const refreshHoveredShape = useCallback(
		(
			...
			if (
				...
				![DRAWCANVAS_CLASSNAME, SELECTIONCANVAS_CLASSNAME].includes(
					e.target.className as string,
				)
			) {
	...
	const updateShape = useCallback(
		(updatedShape: ShapeEntity, withSave = false) => {
			...
			setSelectedShape((prevSelectedShape) =>
				prevSelectedShape?.id === updatedShape.id
					? updatedShape
					: prevSelectedShape,
			);
			withSave && saveShapes();
	...
	const moveShapes = useCallback(
		(firstShapeId: string, lastShapeId: string) => {
			...
			const firstShapeIndex = shapes.findIndex(
				(shape) => shape.id === firstShapeId,
			);
			const lastShapeIndex = shapes.findIndex(
