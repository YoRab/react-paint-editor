...
export const getCursorPosition = (
	...
	return [
		((clientX - canvasBoundingRect.left) *
			(givenWidth / canvasBoundingRect.width)) /
			scaleRatio,
		((clientY - canvasBoundingRect.top) *
			(givenHeight / canvasBoundingRect.height)) /
			scaleRatio,
	];
};
...
const isPartOfRect = (rect: Rect, point: Point, radius: number) =>
	radius
		? isCircleIntersectRect(rect, { x: point[0], y: point[1], radius })
...
export const checkSelectionIntersection = (
	...
	return isPartOfRect(borders, newPosition, radius)
		? { mode: "translate" }
		: false;
};
...
export const checkPositionIntersection = (
	...
	if ("path" in shape && shape.path) {
		...
		if (checkFill) {
			return ctx.isPointInPath(shape.path, newPosition[0], newPosition[1])
				? { mode: "translate" }
				: false;
		}
		ctx.lineWidth = (shape.style?.lineWidth ?? 0) + 2 * radius + 15;

		return ctx.isPointInStroke(shape.path, newPosition[0], newPosition[1])
			? { mode: "translate" }
			: false;
	}

	return checkSelectionIntersection(
		shape,
		position,
		canvasOffset,
		selectionPadding,
