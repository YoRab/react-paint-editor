...
const createBrushPath = (brush: DrawableShape<"brush">) => {
	if (brush.points.length < 1 || brush.style?.strokeColor === "transparent")
		return undefined;
	...
	for (const points of brush.points) {
		if (points.length === 1) {
			path.rect(
				...scalePoint(points[0], minX, minY, brush.scaleX, brush.scaleY),
				1,
				1,
			);
		} else {
			...
			for (const point of points.slice(1)) {
				path.lineTo(
					...scalePoint(point, minX, minY, brush.scaleX, brush.scaleY),
				);
			}
		}
	}
...
export const drawBrush = (
	...
	if (shape.style?.strokeColor === "transparent" || ctx.globalAlpha === 0)
		return;
	ctx.stroke(shape.path);
};

export const getBrushBorder = (
	brush: DrawableShape<"brush">,
	selectionPadding: number,
): Rect => {
	const brushPoints = brush.points.flat();
	...
	const scaledPoints = brushPoints.map((point) =>
		scalePoint(point, minX, minY, brush.scaleX, brush.scaleY),
	);

	const maxX =
		Math.max(...scaledPoints.map((point) => point[0])) + selectionPadding;
	const maxY =
		Math.max(...scaledPoints.map((point) => point[1])) + selectionPadding;
...
export const resizeBrush = (
	...
	const { borders: originalBordersWithoutScale } = getShapeInfos(
		{ ...originalShape, scaleX: 1, scaleY: 1 },
		selectionPadding,
	);
	const { borders: originalBorders } = getShapeInfos(
		originalShape,
		selectionPadding,
	);
	...
	const originalShapeWidth = Math.max(
		0,
		originalBordersWithoutScale.width - 2 * selectionPadding,
	);
	const originalShapeHeight = Math.max(
		0,
		originalBordersWithoutScale.height - 2 * selectionPadding,
	);
