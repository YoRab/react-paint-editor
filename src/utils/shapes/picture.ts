...
import {
	addSizeAndConvertSvgToObjectUrl,
	fetchAndStringify,
	isSvg,
} from "../file";
...
const createFilePicture = (
	img: HTMLImageElement,
	resolve: (
		value: ShapeEntity<"picture"> | PromiseLike<ShapeEntity<"picture">>,
	) => void,
	file: File,
	maxPictureWidth: number,
	maxPictureHeight: number,
	...
	img.onload = async () => {
		...
		const pictureShape = createPictureShape(
			img,
			img.src,
			maxPictureWidth,
			maxPictureHeight,
...
export const getPictureBorder = (
	picture: Picture,
	selectionPadding: number,
): Rect => getRectBorder(picture, selectionPadding);
